...
class SearchController extends Controller
{
    ...
    {
        $this->youtube = new Youtube(['key' => 'AIzaSyCjo0a1CiF7nMt8dS6Nzo__avmRsKR-770']);
    }

    public function search(SearchResultsRequest $request)
    {
        try {
            $validated = $request->validated();
            if ($validated['source'] === 'YouTube') {
                return $this->searchOnYoutube($validated['query']);
            } else {
    ...
    private function searchYoutubeTitle(string $query)
    {
        ...
        $results = array_map(function ($r) {
            $video = $this->youtube->getVideoInfo($r->id->videoId);

            return $this->addResultData($video);
        }, $results);

        return $results;
    }

    private function searchYouTubeUrl($url)
    {
        ...
        array_map(fn($r) => $this->addResultData($r), $results);
        return $results;
    }
    ...
    private function addResultData($infoSource): array
    {
        return [
